...
class ChinaMarketRules:
    ...
    def from_config(cls) -> "ChinaMarketRules":
        return cls(
            ...
            tick_size=float(getattr(ModelConfig, "CN_TICK_SIZE", 0.01)),
        )
    ...
    def build_limit_hit_masks(
        ...
        *,
        ...
        if tick > 0:
            # Exchange limit prices are rounded to the minimum tick size.
            # Using tick-rounded prices reduces boundary misclassification.
            inv = 1.0 / tick
            up_px = torch.floor(prev_day_close * (1.0 + thresholds) * inv + 0.5) / inv
            dn_px = torch.floor(prev_day_close * (1.0 - thresholds) * inv + 0.5) / inv
            limit_up = close >= up_px * (1.0 - tol)
            limit_down = close <= dn_px * (1.0 + tol)
        else:
